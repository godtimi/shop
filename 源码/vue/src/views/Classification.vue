<template>
  <div>
    ...
    <!--弹窗-->
    <el-dialog title="商品种类信息" :visible.sync="dialogFormVisible" width="30%">
...
export default {
  ...
  methods: {
    ...
    del(id) {
      this.request.delete("/classification/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/classification/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    save() {
      this.request.post("/classification", this.form).then(res => {
        if (res.code === '200') {
    ...
    //封装请求数据的方法
    load() {
      this.request.get("/classification/page", {
        params: {
          name: this.name,
